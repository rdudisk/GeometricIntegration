...
template <typename T_SCALAR_TYPE>
std::ostream&
operator<< (std::ostream& stream, Lie::SO3::Algebra<T_SCALAR_TYPE> const& g)
{
	stream << g.v();
	return stream;
}
...
template <typename T_SCALAR_TYPE>
const Lie::SO3::Algebra<T_SCALAR_TYPE>
operator* (T_SCALAR_TYPE s, const Lie::SO3::Algebra<T_SCALAR_TYPE> g) {
	return g*s;
}
