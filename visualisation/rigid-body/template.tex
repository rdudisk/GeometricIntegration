...
\newcommand{\tikzPiOnSphere}[1]{
	\begin{tikzpicture}[>=Stealth,scale=2.5]
		...
		\begin{axis}[hide axis,anchor=origin,x=1cm,y=1cm,unbounded coords=jump]
			\addplot[gray,very thin] table [x=X,y=Y,col sep=comma] {exact-0.csv};
			\addplot[black,very thin] table [x=X,y=Y,col sep=comma] {exact-1.csv};
			\addplot[mark=*,mark size=0.1mm,only marks] table[x=px,y=py,col sep=comma] {#1};
		\end{axis}
		...
		% plot axes
		\def\lambda{1.5}
		\draw[thick,->] (0,0) -- (\pxy{\lambda}{0}{0}) node[anchor=east] {$\pi_1$};
		\draw[thick,->] (0,0) -- (\pxy{0}{\lambda}{0}) node[anchor=west] {$\pi_2$};
		\draw[thick,->] (0,0) -- (\pxy{0}{0}{\lambda}) node[anchor=west] {$\pi_3$};
