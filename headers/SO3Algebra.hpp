...
public:
	Algebra<T> ( )
	: m_v(Eigen::Matrix<T,3,1>::Zero())
	{ }

	Algebra<T> (const Eigen::Matrix<T,3,1>& v)
	: m_v(v)
	{ }
	 ...
	 */
	Algebra<T>
	bracket (const Algebra<T>& g) const
	{
	 ...
	 */
	Eigen::Matrix<T,3,1>
	operator* (const Eigen::Matrix<T,3,1>& vec) const
	{
