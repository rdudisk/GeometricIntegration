...
template <typename T_M,
		  typename T_Q,
		  typename T_TQ>
class EulerStep : public Abstract::Step<T_M,T_Q,T_TQ>
{
private:
...
public:
	EulerStep<T_M,T_Q,T_TQ> (Abstract::Problem<T_M,T_Q>& problem)
	:	Abstract::Step<T_M,T_Q,T_TQ>(problem)
	{
		// m_internals is already initalized by base constructor, overriding
		EulerStepInternals<T_M,T_Q,T_TQ>* internals = new EulerStepInternals<T_M,T_Q,T_TQ>(problem);
		...
		lineSearchParameters.set("Max Iters",1000);
		*/
		...
		EulerStepInternals<T_M,T_Q,T_TQ>* tmp = static_cast<EulerStepInternals<T_M,T_Q,T_TQ>*>(this->m_internals);
	...
	makeStep (void)
	{
		...
		try {
			m_solver->reset(static_cast<EulerStepInternals<T_M,T_Q,T_TQ>*>(this->m_internals)->getInitialGuess());
